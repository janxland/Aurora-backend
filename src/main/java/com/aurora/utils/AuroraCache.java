...
public class AuroraCache {
    ...
    public static void put(String key, Object data, long expire) {
        ...
        if (expire > 0) {
            Future future = executor.schedule(new Runnable() {
                ...
                public void run() {
                    //过期后清除该键值对
                    synchronized (AuroraCache.class) {
                        map.remove(key);
                    }
                }
    ...
    public static String getJedisChache(String key){
        Jedis je = myJedis.getJedis();
        String value = je.get(key);
        je.close();
        return value;
    }
    public static boolean setJedisChache(String key,String value,int seconds){
        Jedis je = myJedis.getJedis();
        try{
            je.setex(key, seconds, value);
            return true;
        } catch(Exception e) {
            return false;
        } finally {
